<div align="center">
  ...
  <p><strong>当前 Pilot 节省 59%–76% 的 Sol Token，同时保持与 Sol 直做相同的 2/2 交付通过结果。</strong></p>
  <p><sub>这个 <code>n=2</code> Pilot 未观察到交付质量下降；它是方向性证据，尚不是普遍性的非劣效结论。</sub></p>
</div>
...
> **Pilot 核心结果：** M3 和 Terra 路线都保持了与 Sol 直接实现相同的 100% 任务成功率（`2/2` 对 `2/2`）和平均质量分（`100` 对 `100`），同时分别减少了 **59.49%** 和 **76.24%** 的昂贵 Sol Token。换句话说，**当前 Pilot 在显著节省 Token 的同时，没有观察到交付质量下降**。
>
> **证据边界：** 当前只有两个方向性测试任务，不能据此断言所有任务都不会受到质量影响。冻结协议要求至少完成 12 个任务对并扩大任务覆盖范围，才能形成发布级非劣效结论。
...
| 路线 | 相对 Sol 直做的成功率 | 观察到的质量变化 | 对照组 Sol Token | 该路线 Sol Token | Sol Token 降幅 | 冻结结论 |
|---|---:|---|---:|---:|---:|---|
| M3 协作 Loop | 100% → 100% | 未观察到下降（`100` → `100`） | 598,925 | 242,649 | **59.49%** | `INSUFFICIENT_SAMPLE` |
| Terra 协作 Loop | 100% → 100% | 未观察到下降（`100` → `100`） | 598,925 | 142,312 | **76.24%** | `INSUFFICIENT_SAMPLE` |
| Claude Sonnet 协作 Loop | 100% → 0% | 质量回退（`100` → `7.5`） | 598,925 | 0 | 无法解释 | `INSUFFICIENT_SAMPLE` |

在这两个 Pilot 任务中，M3 和 Terra 的任务成功率与冻结平均质量分都和 Sol 直做对照组一致，因此当前实验没有观察到交付质量损失。Claude Sonnet 没有消耗 Sol 评审 Token，是因为两个候选结果都没有进入最终评审；这属于质量失败，不能被解释为节省了 100%。
